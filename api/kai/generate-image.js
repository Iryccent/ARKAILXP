/**
 * 🔐 API Route Proxy para generate-image-kai de Supabase
 * 
 * Esta función actúa como proxy entre el frontend y la Supabase Edge Function
 * para evitar problemas de CORS. Llama a la función generate-image-kai de Supabase
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(`${supabaseUrl}/functions/v1/generate-image-kai`, {
      method: 'POST',
      headers: {
    ...
    console.error('❌ Error en proxy de generate-image-kai:', error);
    return res.status(500).json({ 
